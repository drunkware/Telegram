...
public class ChatMediaCell extends ChatBaseCell implements MediaController.FileDownloadProgressListener {
    ...
    protected void onAfterBackgroundDraw(Canvas canvas) {
        ...
        if (infoLayout != null && ((buttonState >= 0 && buttonState < 4) || currentMessageObject.type == 1)) {
            ...
            if (currentMessageObject.type == 1){
                setDrawableBounds(mediaBackgroundDrawable, photoImage.imageX + AndroidUtilities.dp(4), photoImage.imageY + AndroidUtilities.dp(4), infoWidth + AndroidUtilities.dp(8), AndroidUtilities.dpf(16.5f));
            } else {
                setDrawableBounds(mediaBackgroundDrawable, photoImage.imageX + AndroidUtilities.dp(4), photoImage.imageY + AndroidUtilities.dp(4), Math.max(infoWidth , infoWidth2 + infoOffset) + AndroidUtilities.dp(8), 2 * AndroidUtilities.dpf(16.5f));
            }
            mediaBackgroundDrawable.draw(canvas);

            if (currentMessageObject.type == 3) {
                setDrawableBounds(videoIconDrawable, photoImage.imageX + AndroidUtilities.dp(8), 2*(photoImage.imageY + AndroidUtilities.dpf(7.5f)));
